...
/// Database storage adapter used by native tests and alternate host integrations.
public struct HostBackedDatabaseStorage<Host: DatabaseStorageHost>: DatabaseStorage {
    ...
    public func read(key: [UInt8]) throws(DatabaseRuntimeError) -> [UInt8]? {
        ...
        default:
            throw .invalidStorageResponse("Storage host returned a non-read response")
        }
    }
    ...
    public func scan(
        ...
        case .failure(let message):
            throw .storageFailure(message)
        default:
            throw .invalidStorageResponse("Storage host returned a non-scan response")
        }
    }
    ...
    public func commit(_ writes: [DatabaseWriteOperation]) throws(DatabaseRuntimeError) {
        ...
        case .failure(let message):
            throw .storageFailure(message)
        default:
            throw .invalidStorageResponse("Storage host returned a non-commit response")
        }
    }
